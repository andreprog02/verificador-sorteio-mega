...
st.markdown("""
    ...
    .local-tag { font-size: 0.75em; font-weight: bold; color: #555; display: block; margin-bottom: 5px; letter-spacing: 0.5px;}
...
def processar_jogos(texto_jogos):
    apostas_processadas = []
    imagem_atual = "Cupom Desconhecido"

    for linha in texto_jogos.split('\n'):
        linha = linha.strip()
        if not linha: continue
        
        # Identifica cabeçalhos de Cupom ou Arquivo
        if "Cupom:" in linha or "Arquivo:" in linha:
            imagem_atual = linha
            continue

        # Regex para capturar "A: 01 02..." ou "A: 01-02..."
        # Captura letra (opcional) e o restante da linha
        match = re.search(r"^([A-J0-9]+)\s*[:]\s*(.+)$", linha)
        
        if match:
            identificador = match.group(1)
            numeros_texto = match.group(2)
            
            # Extrai todos os números, independente se separados por espaço ou traço
            numeros_lista = re.findall(r'\d+', numeros_texto)
            numeros_set = set(map(int, numeros_lista))
            
            # Só adiciona se tiver pelo menos 6 números
